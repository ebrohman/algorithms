...
def reverseInParentheses(inputString)
  return "" unless inputString.length
  return "" if inputString == "()"

  # inputString.gsub!(/\(\)/, "")

  reversable = inputString.scan(/(?<=\()\.*(?!=\))/).last

  return inputString unless reversable

  index = inputString.index(reversable)

  inputString[(index - 1)..(reversable.length + index)] = reversable.reverse
